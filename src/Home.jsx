...
export default function HomeScreen({
  ...
  useEffect(() => {
    ...
    const u2 = onValue(ref(db,'terapies'),        s => setTer(processFirebaseSnap(s)))
    ...
    <div style={{fontFamily:"-apple-system,'Segoe UI',sans-serif", minHeight:'100vh', background:'#f3f4f7'}}>
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(2,21,63,0.50)',zIndex:2000,display:'flex',alignItems:'flex-end',justifyContent:'center'}}>
          <div style={{background:'#feffff',borderRadius:'24px 24px 0 0',padding:'20px',width:'100%',maxWidth:'480px',maxHeight:'80vh',overflowY:'auto',paddingBottom:'calc(20px + env(safe-area-inset-bottom) + 72px)'}}>
            ...
            {ALL_QUICK_ACTIONS.map(({key,Icon,label,sub,color,grad}) => {
              ...
              return (
                <div key={key}
                  onClick={()=>sel ? saveQA(quickActions.filter(k=>k!==key)) : quickActions.length<3&&saveQA([...quickActions,key])}
            ...
            <button type="button" onClick={()=>setQEdit(false)} style={{width:'100%',padding:'14px',borderRadius:'50px',border:'none',cursor:'pointer',background:'linear-gradient(135deg,#193f9e,#2e84e9)',color:'#fff',fontWeight:'800',fontSize:f(14),marginTop:'8px',fontFamily:'inherit'}}>
              Fatto
            </button>
          </div>
        </div>
        ...
        <div style={{background:'#fdfdfd',borderRadius:'22px',padding:'14px 18px 16px',boxShadow:sh}}>
          ...
          <div style={{display:'flex',justifyContent:'center',marginBottom:'10px'}}>
            <div style={{
              ...
              boxShadow:'0 6px 20px rgba(8,24,76,0.14), 0 2px 8px rgba(0,0,0,0.06)',
          ...
          <div style={{fontSize:f(13),color:'#7c8088',marginBottom:'12px',lineHeight:'1.6',fontStyle:'italic',padding:'8px 12px',background:'#f3f4f7',borderRadius:'12px',borderLeft:'3px solid #2e84e9'}}>
            {frase}
          </div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'10px'}}>
            <button type="button" onClick={()=>go('crisi')} style={{position:'relative',height:'50px',borderRadius:'50px',border:'none',overflow:'hidden',cursor:'pointer',boxShadow:'0 6px 20px rgba(25,63,158,0.35)',touchAction:'manipulation'}}>
            ...
            <button type="button" onClick={()=>go('sos')} style={{height:'50px',borderRadius:'50px',border:'2.5px solid #e53935',background:'#feffff',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:'6px',boxShadow:'0 4px 14px rgba(229,57,53,0.18)',touchAction:'manipulation'}}>
      ...
      <div style={{padding:'0 12px',marginBottom:'10px'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'8px'}}>
          ...
          <div onClick={()=>setQEdit(true)} style={{display:'flex',alignItems:'center',gap:'4px',cursor:'pointer',touchAction:'manipulation'}}>
