...
export default function MessaggiPage({ onBack, isDemo, nomeUtente = 'Famiglia' }) {
  ...
  // ── Carica messaggi real-time ──────────────────────────────
  useEffect(() => {
    if (isDemo) {
      setMessaggi([
        { id: '1', testo: 'Buongiorno! Come sta Damiano questa settimana?', da: 'medico',   mittente: 'Dr. Rossi',  timestamp: Date.now() - 86400000 * 2, letto: true },
        ...
        { id: '3', testo: 'Grazie per l\'aggiornamento. Continuate con la terapia attuale. Se le crisi si ripetono questa settimana, sentitemi subito.', da: 'medico',   mittente: 'Dr. Rossi',  timestamp: Date.now() - 86400000, letto: true },
        ...
        { id: '5', testo: 'Ho rivisto i report delle ultime settimane. Il pattern delle crisi sembra concentrarsi al mattino. Tenete monitorato.', da: 'medico',   mittente: 'Dr. Rossi',  timestamp: Date.now() - 1800000, letto: false },
      ])
      return
    }
    ...
    const unsub = onValue(msgRef, snap => {
      ...
      // Marca come letti i messaggi del medico non ancora letti
      const daLeggere = lista.filter(m => m.da === 'medico' && !m.letto)
      daLeggere.forEach(m => {
    ...
    try {
      if (isDemo) {
        setMessaggi(prev => [...prev, {
          id: Date.now().toString(), testo: txt, da: 'famiglia',
          mittente: nomeUtente, timestamp: Date.now(), letto: false
        }])
        setTesto('')
        setInviando(false)
        return
  ...
  return (
    <div style={{
      ...
      height: '100vh', background: '#f3f4f7',
      ...
      <div style={{
        ...
        {isDemo && (
          <span style={{
            ...
            padding: '3px 10px', fontSize: f(9), color: '#fff', fontWeight: '700'
          }}>DEMO</span>
        )}
      </div>
        ...
        {gruppi.length === 0 && (
          <div style={{
            ...
            <div style={{
              ...
              background: 'linear-gradient(135deg,#193f9e22,#2e84e922)',
        ...
        {gruppi.map((item, i) => {
          if (item.tipo === 'separatore') {
            return (
              <div key={`sep-${i}`} style={{
                ...
                <span style={{
                  background: '#e8eaf0', borderRadius: '20px',
                  padding: '3px 12px', fontSize: f(9),
                  color: '#7c8088', fontWeight: '600'
                }}>{item.giorno}</span>
              </div>
            )
          }
          ...
          return (
            <div key={item.id} style={{
              display: 'flex',
              justifyContent: isMio ? 'flex-end' : 'flex-start',
              marginBottom: '2px'
            }}>
              <div style={{
                maxWidth: '78%',
                ...
                padding: '10px 13px',
                boxShadow: isMio
                  ? '0 4px 14px rgba(25,63,158,0.30)'
                  : '0 2px 10px rgba(2,21,63,0.08)',
              }}>
                {!isMio && (
                ...
                <div style={{
                  fontSize: f(13), lineHeight: '1.5',
                  color: isMio ? '#fff' : '#02153f',
                ...
                <div style={{
                  ...
                  <span style={{ fontSize: f(8), color: isMio ? 'rgba(255,255,255,0.65)' : '#bec1cc' }}>
                    {formatOra(item.timestamp)}
                  </span>
                  {isMio && (
                    item.letto
      ...
      {errore && (
        <div style={{
          margin: '8px 12px 0', padding: '8px 12px', borderRadius: '10px',
          background: '#FEF0F4', border: '1px solid #F7295A22',
        ...
        <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-end' }}>
          ...
          <button
            ...
            style={{
              ...
              background: testo.trim() && !inviando
                ? 'linear-gradient(135deg,#193f9e,#2e84e9)'
                : '#e8eaf0',
