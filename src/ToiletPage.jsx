...
const DEMO_LOG = [
  ...
  {id:6,timestamp:Date.now()-2*86400000,data:'10/04/2026',ora:'08:00',bisogno:'pippi',modalita:'caa-auto',incidentePippi:false,incidenteCacca:false},
  {id:7,timestamp:Date.now()-3*86400000,data:'09/04/2026',ora:'07:45',bisogno:'entrambi',modalita:'adulto',incidentePippi:false,incidenteCacca:false},
  {id:8,timestamp:Date.now()-3*86400000+3600000,data:'09/04/2026',ora:'14:20',bisogno:'pippi',modalita:'caa-auto',incidentePippi:false,incidenteCacca:false},
]
...
export default function ToiletPage({ onBack, isDemo }) {
  ...
  const [filtroStorico, setFiltroStorico] = useState('settimana')
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(ttRef, (snapshot) => {
      const lista = processFirebaseSnap(snapshot).sort((a,b)=>b.timestamp-a.timestamp)
      setLog(lista)
      setLoading(false)
    })
    return () => unsubscribe()
  ...
  function handleSave() {
    ...
    if (hasBagno && !ttModalita) { alert('Seleziona come è andato in bagno'); return }
    const d = new Date(ttDate)
    const dataFmt = `${String(d.getDate()).padStart(2,'0')}/${String(d.getMonth()+1).padStart(2,'0')}/${d.getFullYear()}`
    const sessione = {
      id:Date.now(), timestamp:Date.now(),
      data:dataFmt, ora:ttOra,
      bisogno:ttBisogno||'nessuno', modalita:ttModalita,
      incidentePippi:ttIncidentePippi, oraPippi:ttIncidentePippi?ttOraPippi:'',
      incidenteCacca:ttIncidenteCacca, oraCacca:ttIncidenteCacca?ttOraCacca:'',
    }
    if (!isDemo) push(ref(db,'toilet_training'), encrypt(sessione))
    setSaved(true)
    setTimeout(()=>{
      setSaved(false); setTtBisogno(''); setTtModalita('')
      setTtIncidentePippi(false); setTtOraPippi('')
      setTtIncidenteCacca(false); setTtOraCacca('')
      const n=new Date(); setTtOra(n.toTimeString().slice(0,5))
    },1500)
  }

  function logFiltrato() {
    if (filtroStorico==='oggi') return log.filter(s=>matchOggi(s.data))
    if (filtroStorico==='settimana') return log.filter(s=>(s.timestamp||0)>=Date.now()-7*86400000)
    if (filtroStorico==='mese') return log.filter(s=>(s.timestamp||0)>=Date.now()-30*86400000)
    return log
  }
  ...
  return (
    <>
      <style>{`*{box-sizing:border-box;}body{margin:0;background:#f3f4f7;}.tw{background:#f3f4f7;min-height:100vh;font-family:-apple-system,'Segoe UI',sans-serif;padding-bottom:100px;width:100%;max-width:480px;margin:0 auto;}`}</style>
      <div className="tw">
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',background:'#f3f4f7',margin:'12px 12px 0',borderRadius:'12px',padding:'3px',gap:'3px'}}>
          {[{k:'form',l:'➕ Nuova'},{k:'storico',l:'📋 Storico'},{k:'grafici',l:'📊 Grafici'}].map(({k,l})=>(
            <button key={k} onClick={()=>setSezione(k)} style={{padding:'9px',borderRadius:'9px',border:'none',cursor:'pointer',fontWeight:'700',fontSize:f(11),fontFamily:'inherit',background:sezione===k?'#feffff':'transparent',color:sezione===k?'#7B5EA7':'#7c8088',boxShadow:sezione===k?'0 2px 8px rgba(2,21,63,0.10)':'none',transition:'all 0.2s'}}>
              {l}
            </button>
          ))}
        </div>
            ...
            <>
              <div style={{background:'#feffff',borderRadius:'18px',padding:'14px',marginBottom:'10px',boxShadow:sh}}>
                ...
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px'}}>
                  <div><label style={labelStyle}>Data</label><input type="date" value={ttDate} onChange={e=>setTtDate(e.target.value)} style={inputStyle}/></div>
                  <div><label style={labelStyle}>Ora</label><input type="time" value={ttOra} onChange={e=>setTtOra(e.target.value)} style={inputStyle}/></div>
                </div>
              </div>
              ...
              <div style={{background:'#feffff',borderRadius:'18px',padding:'14px',marginBottom:'10px',boxShadow:sh}}>
                ...
                <div style={{fontSize:f(11),color:'#7c8088',marginBottom:'12px'}}>Registra se c'è stato un incidente — indipendente dal bagno</div>
                {[
                  {val:ttIncidentePippi,set:setTtIncidentePippi,icon:'💧',title:'Pipì addosso',sub:'Ha fatto pipì nei vestiti',ora:ttOraPippi,setOra:setTtOraPippi,labelOra:'Ora incidente pipì'},
                  {val:ttIncidenteCacca,set:setTtIncidenteCacca,icon:'💩',title:'Cacca addosso',sub:'Ha fatto cacca nei vestiti',ora:ttOraCacca,setOra:setTtOraCacca,labelOra:'Ora incidente cacca'},
                ].map(({val,set,icon,title,sub,ora,setOra,labelOra},i)=>(
                  <div key={i}>
                    <div onClick={()=>set(!val)} style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'12px',borderRadius:'14px',cursor:'pointer',background:val?'#FEF0F4':'#f3f4f7',border:`2px solid ${val?'#F7295A33':'transparent'}`,marginBottom:'8px',transition:'all 0.15s'}}>
                    ...
                    {val&&<div style={{padding:'0 4px 10px'}}>
              ...
              <div style={{background:'#feffff',borderRadius:'18px',padding:'14px',marginBottom:'10px',boxShadow:sh}}>
                ...
                <div style={{fontSize:f(11),color:'#7c8088',marginBottom:'12px'}}>Opzionale — compila solo se ha fatto qualcosa in bagno</div>
                <label style={labelStyle}>Cosa ha fatto in bagno</label>
                <div style={{display:'flex',gap:'7px',marginBottom:'14px'}}>
                  {[{key:'pippi',label:'💧 Pipì'},{key:'cacca',label:'💩 Cacca'},{key:'entrambi',label:'🔄 Entrambi'}].map(({key,label})=>(
                    <div key={key} onClick={()=>setTtBisogno(ttBisogno===key?'':key)} style={{flex:1,padding:'10px 6px',borderRadius:'12px',cursor:'pointer',textAlign:'center',fontSize:f(12),fontWeight:'700',border:`2px solid ${ttBisogno===key?'#7B5EA7':'#f0f1f4'}`,background:ttBisogno===key?'#F5F3FF':'#feffff',color:ttBisogno===key?'#7B5EA7':'#7c8088',transition:'all 0.15s'}}>{label}</div>
                  ))}
                </div>
                {ttBisogno&&(
                  <>
                    ...
                    {[
                      ...
                      <div key={opt.key} onClick={()=>setTtModalita(opt.key)} style={{display:'flex',alignItems:'center',gap:'12px',padding:'11px 12px',borderRadius:'12px',cursor:'pointer',marginBottom:'7px',border:`2px solid ${ttModalita===opt.key?'#7B5EA7':'#f0f1f4'}`,background:ttModalita===opt.key?'#F5F3FF':'#feffff',transition:'all 0.15s'}}>
                        <div style={{width:'36px',height:'36px',borderRadius:'10px',background:ttModalita===opt.key?'#7B5EA7':'#f3f4f7',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'18px',flexShrink:0}}>{opt.icon}</div>
                        <div style={{flex:1}}>
              ...
              <button onClick={handleSave} style={{width:'100%',padding:'16px',borderRadius:'50px',border:'none',cursor:'pointer',fontWeight:'800',fontSize:f(15),color:'#fff',background:saved?'linear-gradient(135deg,#00BFA6,#2e84e9)':'linear-gradient(135deg,#7B5EA7,#2e84e9)',boxShadow:'0 6px 20px rgba(123,94,167,0.35)',display:'flex',alignItems:'center',justifyContent:'center',gap:'8px',transition:'all 0.3s',marginBottom:'8px'}}>
                {saved?<><Check size={18} color="#fff"/> Salvato!</>:<><Save size={18} color="#fff"/> Salva sessione</>}
              </button>
              {isDemo&&<div style={{textAlign:'center',fontSize:f(11),color:'#8B6914',fontWeight:'600'}}>🎭 Modalità demo — dati non salvati</div>}
            </>
          )}
            ...
            <>
              <div style={{display:'flex',gap:'6px',marginBottom:'12px'}}>
                {[{k:'oggi',l:'Oggi'},{k:'settimana',l:'7 giorni'},{k:'mese',l:'30 giorni'},{k:'tutto',l:'Tutto'}].map(({k,l})=>(
                  <button key={k} onClick={()=>setFiltroStorico(k)} style={{padding:'6px 12px',borderRadius:'20px',border:'none',cursor:'pointer',fontWeight:'700',fontSize:f(11),fontFamily:'inherit',background:filtroStorico===k?'#7B5EA7':'#feffff',color:filtroStorico===k?'#fff':'#7c8088',boxShadow:filtroStorico===k?'0 3px 10px rgba(123,94,167,0.3)':'0 2px 6px rgba(0,0,0,0.06)',transition:'all 0.2s'}}>
                    {l}
                  </button>
                ))}
              </div>
