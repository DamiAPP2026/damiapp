...
import { ChevronLeft, Copy, Trash2, Check, Plus, Link } from 'lucide-react'
...
// Genera token identico alla vecchia app
...
export default function CondividiPage({ onBack, isDemo }) {
  ...
  const [step, setStep] = useState(0) // 0=lista, 1=wizard, 2=token generato
    ...
    const unsubscribe = onValue(tRef, (snapshot) => {
      ...
      if (val) {
        ...
        Object.entries(val).forEach(([key, encData]) => {
          // Prova a decifrare — compatibile con vecchia e nuova app
  ...
  function handleGenera() {
    ...
    if (!isDemo) {
      // Salva cifrato su Firebase — identico alla vecchia app
      push(ref(db, 'sharetokens'), encrypt(nuovoToken))
    }
  ...
  function handleRevoca(t) {
    ...
    if (!isDemo && t._firebaseKey) {
      // Aggiorna il record cifrato con active: false
      const aggiornato = { ...t, active: false }
      delete aggiornato._firebaseKey
      update(ref(db, `sharetokens/${t._firebaseKey}`), {
        // Sovrascriviamo con il nuovo dato cifrato
        [t._firebaseKey]: encrypt(aggiornato)
      }).catch(() => {
        // Fallback: aggiorna solo il campo active
        update(ref(db, `sharetokens/${t._firebaseKey}`), encrypt({...t, active: false, _firebaseKey: undefined}))
      })
    }
    setTokens(prev => prev.map(tk =>
      tk.id === t.id ? {...tk, active: false} : tk
    ))
  }
  ...
  if (loading) return (
    <div style={{minHeight:'100vh', background:'#f3f4f7', display:'flex',
      ...
      <div className="condividi-wrap">
        ...
        <div style={{
          ...
          <div style={{display:'flex', alignItems:'center', gap:'12px'}}>
            <button
              onClick={step > 0 ? resetWizard : onBack}
              style={{
            ...
            <div>
              ...
              <div style={{fontSize:'11px', color:'rgba(255,255,255,0.75)'}}>
                {step===0 ? 'Gestisci accessi medici'
                  : step===1 ? 'Nuovo accesso medico'
                  : '✅ Token generato!'}
              </div>
            </div>
          </div>
        ...
        <div style={{padding:'12px'}}>
          ...
          {step === 0 && (
            <>
              ...
              {/* Token attivi */}
              <div style={{
                fontSize:'13px', fontWeight:'800', color:'#02153f', marginBottom:'8px'
              }}>
              ...
              {activeTokens.length === 0 ? (
                <div style={{
                  ...
                  textAlign:'center', boxShadow:shSm, marginBottom:'10px'
                }}>
                  <div style={{fontSize:'28px', marginBottom:'8px'}}>🔗</div>
                  <div style={{fontSize:'13px', color:'#7c8088'}}>Nessun token attivo</div>
                  <div style={{fontSize:'11px', color:'#bec1cc', marginTop:'4px'}}>
                    Genera un token per condividere i dati con il medico
                  </div>
                </div>
              ) : (
                activeTokens.map(t => {
                  const scadenza = t.expiresAt ? new Date(t.expiresAt) : null
                  const ggRimanenti = scadenza
                    ? Math.ceil((scadenza - Date.now()) / 86400000)
                    : null
                  return (
                    <div key={t.id} style={{
                      ...
                      <div style={{
                        ...
                        <div>
                          <div style={{
                            fontSize:'14px', fontWeight:'800', color:'#02153f', marginBottom:'4px'
                          }}>
                            👨‍⚕️ {t.medicoName}
                          </div>
                          <div style={{
                            ...
                            color:'#193f9e', letterSpacing:'2px'
                          }}>{t.token}</div>
                        </div>
                      ...
                      <div style={{display:'flex', gap:'5px', flexWrap:'wrap', marginBottom:'8px'}}>
                        ...
                        {t.permissions?.shareTerapie && (
                          <span style={{fontSize:'10px', fontWeight:'700', padding:'2px 8px',
                            borderRadius:'20px', background:'#E8FBF8', color:'#00BFA6'}}>
                            Terapie
                          </span>
                        )}
                        {t.permissions?.shareDocuments && (
                          <span style={{fontSize:'10px', fontWeight:'700', padding:'2px 8px',
                            borderRadius:'20px', background:'#EEF3FD', color:'#193f9e'}}>
                            Documenti
                          </span>
                        )}
                      ...
                      <div style={{
                        fontSize:'11px',
                        color: ggRimanenti !== null && ggRimanenti < 10 ? '#F7295A' : '#7c8088'
                      }}>
                        ⏳ {ggRimanenti !== null
                          ? `Scade tra ${ggRimanenti} giorni`
                          : 'Nessuna scadenza'}
                      </div>
                      ...
                      <button
                        ...
                        style={{
                          ...
                          background:'#EEF3FD', color:'#193f9e',
              ...
              {revokedTokens.length > 0 && (
                <>
                  <div style={{
                    fontSize:'13px', fontWeight:'800', color:'#bec1cc',
                    margin:'12px 0 8px'
                  }}>
                  ...
                  {revokedTokens.map(t => (
                    <div key={t.id} style={{
                      background:'#f3f4f7', borderRadius:'14px',
                      padding:'12px', marginBottom:'6px', opacity:0.6
                    }}>
                      ...
                      <div style={{
                        fontFamily:"'Courier New', monospace",
                        fontSize:'13px', color:'#bec1cc', marginTop:'2px'
                      }}>{t.token}</div>
          ...
          {step === 1 && (
            <div style={{
              ...
              padding:'16px', boxShadow:sh
            }}>
              <div style={{
                fontSize:'13px', fontWeight:'800', color:'#02153f', marginBottom:'16px'
              }}>
                👨‍⚕️ Dati del medico
              </div>
              ...
              <div style={{marginBottom:'16px'}}>
                ...
                {[
                  ...
                  <div
                    key={key}
                    onClick={() => !locked && setPerms(p => ({...p, [key]:!p[key]}))}
                    style={{
                    ...
                    <div>
                      <div style={{
                        ...
                        color: perms[key] ? '#193f9e' : '#394058'
                      }}>{label}</div>
                      {locked && (
                        <div style={{fontSize:'10px', color:'#bec1cc'}}>
                          Sempre incluso
                        </div>
                      )}
                    </div>
          ...
          {step === 2 && tokenGenerato && (
            <div style={{
              ...
              <div style={{textAlign:'center', marginBottom:'20px'}}>
                ...
                <div style={{
                  fontSize:'18px', fontWeight:'900', color:'#08184c', marginBottom:'4px'
                }}>
                  Token generato!
                </div>
              ...
              <div style={{
                ...
                <div style={{
                  fontFamily:"'Courier New', monospace",
                  fontSize:'24px', fontWeight:'900', color:'#193f9e',
              ...
              {/* Copia */}
              <button
                onClick={() => handleCopy(tokenGenerato.token)}
                style={{
                  ...
                  display:'flex', alignItems:'center', justifyContent:'center',
                  gap:'8px', marginBottom:'10px', transition:'all 0.2s'
                }}>
                {copied
                  ? <><Check size={16} color="#fff"/> Copiato!</>
                  : <><Copy size={16} color="#193f9e"/> Copia token</>
                }
              </button>
