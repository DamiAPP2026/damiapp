...
import {
  ...
  Activity, Package, BookOpen, Shield, BarChart3, AlertCircle
} from 'lucide-react'
...
const PERIODI = [
  { key: 'all',     label: 'Tutto' }, { key: 'week',    label: '7g'  },
  { key: 'month',   label: '30g'  }, { key: '3months', label: '3M'  },
  { key: '6months', label: '6M'   }, { key: 'year',    label: '1A'  },
]
...
// ─── CHAT MEDICO ──────────────────────────────────────────────
// Ogni medico legge/scrive SOLO su messages/{tokenId}
// così ogni chat è separata e privata.
function ChatMedico({ tokenData }) {
  ...
  // Percorso Firebase dedicato a questo token
  const chatPath = `messages/${tokenData.token}`
  ...
  useEffect(() => {
    const unsub = onValue(ref(db, chatPath), snap => {
      ...
      lista
        .filter(m => m.da === 'famiglia' && !m.letto)
  ...
  async function invia() {
    ...
    try {
      await push(ref(db, chatPath), {
        testo: txt, da: 'medico', mittente: nomeMedico,
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 130px)' }}>
      <div style={{ margin: '12px 12px 0', padding: '8px 14px', background: 'linear-gradient(135deg,#EEF3FD,#E8FBF8)', borderRadius: '12px', display: 'flex', alignItems: 'center', gap: '8px', border: '1px solid #d8e8f8', flexShrink: 0 }}>
        ...
        {nonLettiF > 0 && (
          <span style={{ marginLeft: 'auto', background: '#F7295A', color: '#fff', borderRadius: '20px', padding: '2px 8px', fontSize: f(9), fontWeight: '800' }}>
            {nonLettiF} nuovi
          </span>
        )}
      </div>
      ...
      <div style={{ background: '#FFF5EE', borderTop: '1px solid #FF8C4222', padding: '6px 14px', fontSize: f(9), color: '#8B6914', lineHeight: '1.5', flexShrink: 0 }}>
        ⚠️ Messaggi <strong>permanenti e non eliminabili</strong>. Non sostituisce una visita medica.
      </div>
...
export default function DoctorView({ tokenData, onLogout }) {
  ...
  // Path Firebase dedicato a questo token medico
  const chatPath = `messages/${tokenData.token}`

  useEffect(() => { loadData() }, [])

  // Conta solo i messaggi non letti della PROPRIA chat (messages/{tokenId})
  useEffect(() => {
    const unsub = onValue(ref(db, chatPath), snap => {
      const val = snap.val()
  ...
  if (loading) return (
    <div style={{ minHeight: '100vh', background: '#f3f4f7', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "-apple-system,'Segoe UI',sans-serif" }}>
      <div style={{ textAlign: 'center' }}>
        <div style={{ fontSize: '32px', marginBottom: '12px' }}>👨‍⚕️</div>
        <div style={{ fontSize: f(14), color: '#7c8088' }}>Caricamento dati paziente...</div>
      </div>
    </div>
  )
  ...
  return (
    <>
      ...
      <div className="doc-wrap">
        ...
        {/* ── TAB REPORT ── */}
        {tab === 'report' && <ReportPage isDemo={false} onBack={() => setTab('dati')} />}
        ...
        {tab === 'dati' && (
          <div style={{ padding: '12px' }}>
            ...
            <div onClick={() => setTab('report')} style={{ background: 'linear-gradient(135deg,#193f9e,#2e84e9)', borderRadius: '16px', padding: '14px 16px', marginBottom: '10px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '12px', boxShadow: '0 6px 20px rgba(25,63,158,0.30)' }}>
